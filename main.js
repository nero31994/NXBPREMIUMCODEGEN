...
document.addEventListener("DOMContentLoaded", async function () {
    const m3uUrl = "https://combinem3u.vercel.app/api/combine-m3u";
    const storageKey = "sM3uList";
    const lastDownloadKey = "m3uLastDownload";
    ...
    function fetchAndLoadM3U() {
        localStorage.setItem(storageKey, m3uUrl);
        localStorage.setItem(lastDownloadKey, now);
    ...
    function checkAndUpdateM3U() {
        let lastDownload = localStorage.getItem(lastDownloadKey);
        if (!lastDownload || now - lastDownload > oneDay) {
            console.log("Downloading a fresh M3U file...");
            fetchAndLoadM3U();
        } else {
            console.log("Using cached M3U file.");
            document.getElementById("sM3uUrl").value = localStorage.getItem(storageKey);
            setTimeout(() => {
                playlistReadyHandler();
                setTimeout(() => {
                    loadChannel(1);
    ...
    async function verifyLogin(code) {
        try {
            const response = await fetch(`https://generate.arenaofvalorph937.workers.dev/verify?code=${encodeURIComponent(code)}`);
            const result = await response.json();

            if (result.success) {
                localStorage.setItem(authTokenKey, code);
                hideLoginScreen();
                checkAndUpdateM3U();

                // Schedule logout when code expires
                if (result.expiresAt) {
                    const expiresInMs = result.expiresAt - Date.now();
                    setTimeout(logout, expiresInMs);
                }
            } else {
                loginMessage.textContent = "Invalid or expired code.";
                logout();
            }
        } catch (error) {
    ...
    if (loginForm) {
        loginForm.addEventListener("submit", function (e) {
            ...
            if (!code) {
                loginMessage.textContent = "Please enter a code.";
                return;
            }
            verifyLogin(code);
        });
    ...
    const storedCode = localStorage.getItem(authTokenKey);
    if (storedCode) {
        verifyLogin(storedCode);
    } else {
        showLoginScreen();
    }
    ...
    let advancedSettingsTab = document.getElementById("settings_advanced_tab");
    if (advancedSettingsTab) {
        advancedSettingsTab.onclick = function () {
            alert("Access Restricted!");
            return false;
        };
    }
